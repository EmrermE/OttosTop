/**
 * Akıllı Taksi/Kargo Çağırma ve Rota Optimizasyonu Simülasyonu - İstemci Mantığı
 * Canvas Üzerinde Harita Çizimi, Canlı Rota Animasyonları ve API Yönetimi
 */
...
window.addEventListener("load", () => {
    ...
    // --- DOKUNMATIK EKRAN DESTEĞİ (Touch Events for Mobile Pan & Pinch-Zoom) ---
    let lastTouchDist = 0;
    ...
    // Ekran üzeri zoom buton kontrol olayları
    ...
    if (btnZoomOut) {
        btnZoomOut.addEventListener("click", () => {
            zoomScale = Math.max(zoomScale - 0.15, 0.8);
            clampPanOffset();
            drawMap();
        });
    }
...
function updateUI(fillDropdowns = false) {
    ...
    // Always update vehicle select list to reflect their current locations,
    // but preserve the user's active selection!
...
// --- YOL BULMA VE MESAFE HESAPLAMA YARDIMCI FONKSİYONLARI ---
...
// Tema Renk Paletleri (Canvas Çizimi İçin Tarayıcı Uyumlu Değerler)
const THEME_COLORS = {
    light: {
        ...
        roadActiveTrip: "rgba(52, 199, 89, 0.95)", // Beautiful vibrant green!
        textActive: "rgba(52, 199, 89, 1)",
        textInactive: "rgba(0, 0, 0, 0.45)",
        sea: "#bae6fd",            // Yumuşak Marmara/Boğaz Mavisi (Açık Tema)
        coast: "rgba(14, 165, 233, 0.25)",
        land: "#f3f4f6",           // Açık Gri Modern Kara Parçası
    ...
    dark: {
        ...
        roadActiveTrip: "rgba(16, 185, 129, 0.95)", // Beautiful glowing emerald green!
        textActive: "#38bdf8",
        textInactive: "rgba(255, 255, 255, 0.35)",
        sea: "#0f172a",            // Derin Gece Mavisi Deniz (Koyu Tema)
        coast: "rgba(56, 189, 248, 0.12)",
        land: "#0b0f19",           // Ultra Premium Koyu Arayüz Karası
...
function drawMap(highlightedRoutes = systemState.lastMatchedRoute || { pickup: [], trip: [] }, animVehicle = null) {
    ...
    // 1. ADIM: Deniz (Sea) Arka Plan Çizimi — Tüm canvas deniz rengiyle doldurulur
    // Bu sayede pan/zoom yapıldığında deniz her yerde sabit kalır
    ctx.fillStyle = theme.sea;
    ctx.fillRect(0, 0, canvas.width, canvas.height);

    // Deniz üzerine ince dalgalanma efekti (Wave shimmer overlay)
    const waveGrad = ctx.createLinearGradient(0, canvas.height * 0.6, 0, canvas.height);
    waveGrad.addColorStop(0, "transparent");
    waveGrad.addColorStop(1, isLight ? "rgba(14, 165, 233, 0.06)" : "rgba(56, 189, 248, 0.03)");
    ...
    // Galata Köprüsü kaldırıldı (Mahalle ölçeğine uygun olarak)
...
function startRouteAnimation(step) {
    ...
    activeAnimation = {
        ...
        progress: 0,
        phase: 'pickup',
        speed: 0.001, // 3 times slower, smooth GPS-like flow
        angle: startAngle,
